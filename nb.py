...
def startWallet():
    ...
    background(args.keosd + ' --unlock-timeout %d --http-server-address 120.197.130.116:6666 --wallet-dir %s' % (unlockTimeout, os.path.abspath(args.wallet_dir)))
    sleep(.4)
...
def startNode(nodeIndex, account):
    ...
    run('rm -rf ' + dir)
    run('mkdir -p ' + dir)
    otherOpts = ''.join(list(map(lambda i: '    --p2p-peer-address 120.197.130.116:' + str(9000 + i), range(nodeIndex))))
    ...
    cmd = (
        ...
        '    --http-server-address 120.197.130.116:' + str(8000 + nodeIndex) +
        '    --p2p-listen-endpoint 120.197.130.116:' + str(9000 + nodeIndex) +
...
def randomTransfer(b, e):
    for j in range(20):
        ...
        run(args.cleos + 'transfer -f ' + src + ' ' + dest + ' "0.0001 ' + args.eossymbol + '"' + ' || true')
...
parser.add_argument('--cleos', metavar='', help="Cleos command", default='/data4/eeos/build/programs/cleos/cleos --wallet-url http://120.197.130.116:6666 ')
parser.add_argument('--nodeos', metavar='', help="Path to nodeos binary", default='/data4/eeos/build/programs/nodeos/nodeos')
parser.add_argument('--keosd', metavar='', help="Path to keosd binary", default='/data4/eeos/build/programs/keosd/keosd')
parser.add_argument('--contracts-dir', metavar='', help="Path to contracts directory", default='/data4/eeos/build/contracts/')
...
args.cleos += '--url http://120.197.130.116:%d ' % args.http_port
